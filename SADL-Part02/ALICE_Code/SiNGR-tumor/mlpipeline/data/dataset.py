...
class DataFrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # print("INISDE GETITEM IN DATAFRAME DATASET")
...
class BRATSDataset(DataFrameDataset):
    ...
    def read_data(self, entry):
        ...
        patient_id = f"BraTS20_Training_{int(entry['seg']):03d}"
        base_dir = os.path.join(self.root.brats, patient_id)

        fullname_gt = os.path.join(base_dir, f"{patient_id}_seg.nii")
        ...
        for inp in input_names:
            fullname_input = os.path.join(base_dir, f"{patient_id}_{inp}.nii")
            ...
            if self.use_geo:
                geo_path = os.path.join(self.root.geo, patient_id, f"{patient_id}_seg_transformed.nii.gz")
                geo_gt = nib.load(geo_path).get_fdata()
                # print("GEO GT SHAPE:", geo_gt.shape)
                geo_gts.append(geo_gt)

        images = np.stack(images, axis=0)
        
        if self.use_geo and len(geo_gts) > 0:
            geo_gts = np.stack(geo_gts, axis=0)  # (N_modalities, H, W, D)
            geo_gts = np.mean(geo_gts, axis=0)   # (H, W, D)
            geo_gts = geo_gts + 1.0
            geo_gts = np.expand_dims(geo_gts, axis=0).astype(np.float32)  # (1, H, W, D)

            # print(geo_gts.shape)
            # print(geo_gts[0])
            geo_gts = np.transpose(geo_gts, [3, 0, 1, 2])
            transformed_dict = {'image': images, 'label': gts, 'dist': geo_gts}
        else:
            transformed_dict = {'image': images, 'label': gts}

        # if self.use_geo and geo_gts:
        #     geo_gts = np.stack(geo_gts, axis=-1)
        #     geo_gts = np.mean(geo_gts, axis=-1) + 1.0
        #     geo_gts = np.transpose(geo_gts, [3, 0, 1, 2])
        #     transformed_dict = {'image': images, 'label': gts, 'dist': geo_gts}
        # else:
        #     transformed_dict = {'image': images, 'label': gts}
    ...
    #     input_names = ['t1', 't2', 'flair', 't1ce']
    #     target_name = 'seg'
    #     print("ENTRY COLUMNS:", entry.index)
    #     print("TARGET NAME BEING USED:", target_name)
    #     fullname_gt = os.path.join(self.root.brats, entry[target_name])
    #     print("FULLNAME GT:", fullname_gt)
    #     gts = nib.load(fullname_gt).get_fdata()
    #     images = []
    #     input_filenames = []
    #     geo_gts = []
...
''' Second entry which kinda worked
    ...
    def read_data(self, entry):
        ...
        for inp in input_names:
            ...
            if self.label_name and self.use_geo:
                geo_path = os.path.join(self.root.geo, patient_id, f"{patient_id}_seg_transformed.nii.gz")
                geo_gt = nib.load(geo_path).get_fdata()
                print("GEO GT SHAPE:", geo_gt.shape)
                geo_gts.append(geo_gt)
