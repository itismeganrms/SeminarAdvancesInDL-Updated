import nibabel as nib
import numpy as np
from pathlib import Path
import pandas as pd
import os
import click
import pickle
import random
from natsort import natsorted

import mlpipeline
# from mlpipeline import *
from mlpipeline.data import *
from mlpipeline.data.splitter import FoldSplit


def generate_metadata(root_dir):
    dirs = [d for d in os.listdir(root_dir) if os.path.isdir(root_dir / Path(d))]
    # suffices = ['flair_0', 'flair_1', 'flair_2', 'seg']
    suffices = ['mask']
    data = []
    for dir_name in dirs:
        id = dir_name
        row = {'seg': id}
        for suffix in suffices:
            fullname = root_dir / Path(dir_name) / Path(f"{id}_{suffix}.tif")
            if os.path.isfile(fullname):
                row[suffix] = Path(dir_name) / Path(f"{id}_{suffix}.tif")
        data.append(row)

    data = pd.DataFrame(data)
    return data


def split_train_test(df, output_dir, target_col=None, group_col=None, seed=28, n_folds=5):
    # Split train vs test
    splitter = FoldSplit(ds=df, n_folds=n_folds, target_col=target_col, group_col=group_col, random_state=seed)
    df_train_val, df_test = splitter.fold(0)

    # Split k-fold
    data = []
    output_fullname = os.path.join(output_dir, f"cv_split_{n_folds}folds_lgg_{seed:05d}.pkl")
    splitter_kf = FoldSplit(ds=df_train_val, n_folds=n_folds, target_col=target_col, group_col=group_col, random_state=seed)
    for fold_ind in range(n_folds):
        df_train, df_val = splitter_kf.fold(fold_ind)
        data.append((df_train, df_val))
        print(f"Fold {fold_ind}, Dataset: LGG, Size: {len(df_train_val)} to {len(df_train)}/{len(df_val)}")

    with open(output_fullname, "wb") as f:
        print(f"Saving metadata to {output_fullname}")
        pickle.dump(data, f, protocol=4)

    # Save test data
    output_fullname = os.path.join(output_dir, f"cv_split_{n_folds}folds_lgg_test_{seed:05d}.pkl")
    print(len(df_test))
    with open(output_fullname, "wb") as f:
        print(f"Saving metadata to {output_fullname}")
        pickle.dump(df_test, f, protocol=4)


@click.command()
@click.option("--root")
@click.option("--output_dir")
@click.option("--seed", default=99999)
def main(root, output_dir, seed):
    dataset = 'lgg'
    print(f'Seed: {seed}')
    metadata_fullname = Path(output_dir) / f'{dataset}_metadata_{seed}.csv'
    df = generate_metadata(root)
    print(f'Saving metadata to {metadata_fullname}')
    df.to_csv(metadata_fullname, index=None)

    # Generate seed
    if seed == -1:
        seed = random.randint(1, 99999)

    # Data
    n_folds = 5
    split_train_test(df, output_dir=output_dir, n_folds=n_folds, seed=seed)


if __name__ == "__main__":
    main()
