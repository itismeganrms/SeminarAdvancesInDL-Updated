...
# Add the parent directory of 'mlpipeline' to sys.path
...
from mlpipeline import utils, train
...
def main():
    ...
    conf_path = cwd / "workdir" / "config" / "config.yaml"
    print(f"Config path: {conf_path}")
    cfg = init_conf(cwd, conf_path)
    # cfg.seed = print(str(42).zfill(3))
    # print()
    print(f"Cfg : {cfg}")
