...
def main():
    ...
    train_files = get_data_with_geo(args.data_dir, geo_dir, geo_map_dir, train_ids)
    val_files = get_data_with_geo(args.data_dir, geo_dir, geo_map_dir, val_ids)
    patch_size = (64, 64, 64)
    ...
    with torch.no_grad():
        ...
        for val_data in val_loader:
            # image = val_data["image"].to(device)
            # geo = val_data["geo"][:, :4].to(device)
            # val_inputs = torch.cat((val_data["image"].to(device), geo), dim=1)
            image = val_data["image"].to(device)
            geo = val_data["geo"][:, :4].to(device)
            val_inputs = torch.cat((image, geo), dim=1)
            val_labels = val_data["label"].to(device)
