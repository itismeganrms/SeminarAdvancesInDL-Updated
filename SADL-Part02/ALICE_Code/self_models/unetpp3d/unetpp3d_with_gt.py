...
def get_data_with_geodesic(data_dir, geo_dir, id_list):
    input_names = ['t1', 't2', 'flair', 't1ce']
    data = []
    for id_num in id_list:
        patient_id = f"BraTS20_Training_{int(id_num):03d}"
        ...
        images = [os.path.join(base_dir, f"{patient_id}_{mod}.nii") for mod in input_names]
        label = [os.path.join(geo_label_dir, f"{patient_id}_{mod}.nii") for mod in input_names]
        if not all(os.path.exists(img) for img in images) or not os.path.exists(label):
            continue
        data.append({"image": images, "label": label})
    return data
...
def main():
    ...
    parser.add_argument("--geo_dir", type=str, default=None, help="If provided, use geodesic-transformed labels from this directory")
    ...
    wandb.init(
        ...
        config={
            ...
            "optimizer": "AdamW",
            "lr": 1e-4,
            "label_type": "geodesic" if args.geo_dir else "original"
        },
        tags=["UNet++", "3D","geodesic-maps"]
    )
    ...
    # if args.geo_dir:
    train_files = get_data_with_geodesic(args.data_dir, args.geo_dir, train_ids)
    val_files = get_data_with_geodesic(args.data_dir, args.geo_dir, val_ids)
    print("Using geodesic-transformed labels.", flush=True)
    # else:
    #     train_files = get_data(args.data_dir, train_ids)
    #     val_files = get_data(args.data_dir, val_ids)
    #     print("Using original labels.", flush=True)
